...
- **Unix only** — RSB targets Linux/Unix exclusively. Do not add `#[cfg(windows)]` blocks, Windows-specific code paths, or Windows compatibility shims. Use Unix-specific APIs (e.g., `std::os::unix::fs::PermissionsExt`) directly without `#[cfg(unix)]` guards.
