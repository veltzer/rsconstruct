...
Tools found in Makefiles across `../*/` sibling projects that rsb does not yet have processors for.
...
### check-jsonschema
- **What it does:** Validates YAML/JSON files against JSON Schema (distinct from rsb's json_schema which validates JSON against schemas found via `$schema` key).
...
- **Notes:** rsb's built-in C/C++ dependency analyzer already handles this.
