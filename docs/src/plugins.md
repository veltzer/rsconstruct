...
A plugin name must not conflict with a built-in processor name (`tera`, `ruff`, `pylint`, `cc_single_file`, `cppcheck`, `shellcheck`, `spellcheck`, `sleep`, `make`). RSConstruct will error if a conflict is detected.
