...
fn setup_project_with_template() -> tempfile::TempDir {
    ...
    fs::write(project_path.join("config/test.py"), "value = 1").expect("Failed to write config");
    fs::write(
...
fn graph_mermaid_format() {
    ...
    let output = run_rsconstruct_with_env(
        project_path,
        &["graph", "show", "--format", "mermaid"],
        &[("NO_COLOR", "1")],
    );
    assert!(
        output.status.success(),
        "graph --format mermaid failed: {}",
        String::from_utf8_lossy(&output.stderr)
    );
    ...
    assert!(
        ...
        "Expected mermaid graph syntax in output, got: {}",
        stdout
    );
}
...
fn graph_text_format() {
    ...
    let output = run_rsconstruct_with_env(
        project_path,
        &["graph", "show", "--format", "text"],
        &[("NO_COLOR", "1")],
    );
    assert!(
        output.status.success(),
        "graph --format text failed: {}",
        String::from_utf8_lossy(&output.stderr)
    );
...
fn graph_stats() {
    ...
    assert!(
        ...
        "Expected 'products' in stats output, got: {}",
        stdout
    );
}
...
fn graph_unreferenced_finds_untracked_files() {
    ...
    assert!(
        output.status.success(),
        "graph unreferenced failed: {}",
        String::from_utf8_lossy(&output.stderr)
    );

    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("unused.txt"),
        "Should report unused.txt: {}",
        stdout
    );
    assert!(
        !stdout.contains("src/used.txt"),
        "Should not report src/used.txt: {}",
        stdout
    );
}
...
fn graph_unreferenced_dep_inputs_not_reported() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        !stdout.contains("dep.txt"),
        "dep.txt is referenced via dep_inputs, should not appear: {}",
        stdout
    );
    assert!(
        !stdout.contains("source.txt"),
        "source.txt is a primary input, should not appear: {}",
        stdout
    );
    assert!(
        stdout.contains("unused.txt"),
        "unused.txt should appear: {}",
        stdout
    );
}
...
fn graph_unreferenced_rm_deletes_files() {
    ...
    assert!(
        !project_path.join("unused.txt").exists(),
        "unused.txt should have been deleted"
    );
    assert!(
        project_path.join("src/used.txt").exists(),
        "src/used.txt should still exist"
    );
}
