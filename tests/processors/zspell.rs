...
fn zspell_misspelled_word() {
    ...
    assert!(
        combined.contains("speling"),
        "Error should mention the misspelled word: {}",
        combined
    );
}
...
fn zspell_incremental_skip() {
    ...
    assert!(
        stdout2.contains("[zspell] Skipping (unchanged):"),
        "Second build should skip: {}",
        stdout2
    );
}
...
fn zspell_stops_after_first_error() {
    ...
    let output =
        run_rsconstruct_with_env(project_path, &["build", "-j", "1"], &[("NO_COLOR", "1")]);
    assert!(
        !output.status.success(),
    ...
    assert!(
        combined.contains("speling"),
        "Should report the error from the first file (aaa.md): {}",
        combined
    );
    assert!(
        !combined.contains("diferent"),
        "Should NOT report the error from the second file (bbb.md) — \
         rsconstruct stops after the first failure: {}",
        combined
    );
}
...
fn zspell_auto_add_words() {
    ...
    assert!(
        words_content.contains("rsconstruct"),
        "Should contain 'rsconstruct': {}",
        words_content
    );
    assert!(
        words_content.contains("tera"),
        "Should contain 'tera': {}",
        words_content
    );
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("Added") && stdout.contains("word"),
        "Should mention adding words: {}",
        stdout
    );
}
