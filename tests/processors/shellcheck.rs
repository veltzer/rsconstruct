...
#[test]
fn shellcheck_valid_script() {
    if !tool_available("shellcheck") {
        eprintln!("shellcheck not found, skipping test");
        return;
    }
    ...
    assert!(
        ...
        "Build should succeed with valid shell script: stdout={}, stderr={}",
    ...
    assert!(
        ...
        "Should process shellcheck: {}",
        stdout
    );
}

#[test]
fn shellcheck_incremental_skip() {
    if !tool_available("shellcheck") {
        eprintln!("shellcheck not found, skipping test");
        return;
    }
    ...
    assert!(
        stdout2.contains("[shellcheck] Skipping (unchanged):"),
        "Second build should skip: {}",
        stdout2
    );
}
