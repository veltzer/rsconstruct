...
fn marp_ci_cap_sets_max_jobs_when_ci_true() {
    ...
    assert_eq!(
        max_jobs.as_i64(),
        Some(2),
        "expected max_jobs=2 under CI=true, got {:?}",
        max_jobs
    );
}
...
fn marp_ci_cap_absent_when_ci_unset() {
    ...
    assert!(
        ...
        "expected max_jobs absent without CI=true, got {:?}",
        max_jobs
    );
}
...
fn marp_ci_cap_respects_user_set_max_jobs() {
    ...
    assert_eq!(
        max_jobs,
        Some(5),
        "user-set max_jobs must win over CI cap; got {:?}",
        max_jobs
    );
}
...
#[test]
fn marp_nonexistent_src_dir_is_config_error() {
    let temp_dir = TempDir::new().expect("Failed to create temp dir");
    let project_path = temp_dir.path();
    ...
    assert!(
        stderr.contains("processor.marp") && stderr.contains("marp"),
