...
fn create_tidy(toml: &toml::Value) -> anyhow::Result<Box<dyn crate::processors::Processor>> {
    crate::registries::deserialize_and_create(toml, |cfg| {
        Box::new(SimpleChecker::new(
            cfg,
            SimpleCheckerParams {
                description: "Validate HTML files using tidy",
                subcommand: Some("-errors"),
                prepend_args: &[],
                extra_tools: &[],
                fix_subcommand: None,
                fix_prepend_args: &[],
...
inventory::submit! { crate::registries::ProcessorPlugin {
    ...
    defaults: Some(crate::config::ProcessorDefaults { command: "tidy", ..crate::config::ProcessorDefaults::EMPTY }),
